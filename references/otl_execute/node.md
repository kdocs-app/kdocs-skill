...
**IBlock**（块级节点）：`title`、`codeBlock`、`paragraph`、`heading`、`blockQuote`、`highLightBlock`、`lockBlock`、`table`、`column`、`pictureColumn`、`picture`、`blockAnchor`、`hr`、`video`、`audio`、`videoLink`、`countdown`、`map`、`thirdResource`、`dbsheet`、`spreadsheet`、`groupCard`、`appComponent`、`processon`
...
|------|------|------|------|
| `url` | string | **是** | 视频资源链接 |
| `platform` | integer | 否 | 平台：`1`=bilibili，默认 `1` |
| `align` | integer | 否 | 水平对齐 — `1`=左对齐, `2`=居中, `3`=右对齐，默认 `1` |
...
| `platform` | integer | 否 | 平台：`1`=高德地图，默认 `1` |
| `mapZoom` | integer | 否 | 缩放尺度 |
| `align` | integer | 否 | 水平对齐，默认 `1` |
...
|------|------|------|------|
| `url` | string | **是** | 资源链接 |
| `type` | integer | 否 | `1`=慕客, `2`=墨刀, `3`=masterGo, `4`=网易云音乐, `5`=阿里云盘视频, `6`=意向收集, `7`=figma, `8`=pixso, `9`=小画桌，默认 `1` |
| `align` | integer | 否 | 水平对齐，默认 `1` |
