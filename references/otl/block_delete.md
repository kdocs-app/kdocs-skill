...
调用 `otl.block_delete` 工具，按父块和索引范围删除内容，适合局部删除。

## 典型用法

1. **清空文档标题**：blockId 设置为 "doc"，startIndex=0，endIndex=1
2. **删除某个块的子节点**：blockId 设置为通过查询得到的块 id，确定好 startIndex 和 endIndex
