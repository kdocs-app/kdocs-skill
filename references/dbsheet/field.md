...
> 部分接口要求 Header `Content-Type: application/json` 及鉴权相关头（由运行时处理）；Agent 勿在对话中拼接 Authorization 明文。
> 选项类字段的选项定义在 `data.items` 内；与旧文档中把 `items` 写在字段根级的写法不同，以本接口 JSON 为准。
> 身份证字段类型名为 `ID`；部分历史示例写作 `Id`，以网关校验为准。
...
- `fields` (array, 必填): 待创建字段列表；每项为对象，至少含 `name`、`type`，多数类型需额外 `data`（结构因类型而异，见 param_detail）
  ...
  - `type` (string, 必填): 字段类型枚举（见 param_detail 完整列表；如 `Date`、`ID`、`Percent`/`Percentage` 等以网关校验为准）
...
类型名以网关为准（文档中常见 `Percent` 与 `Percentage` 两种写法）。
